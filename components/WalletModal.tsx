            ...
            <div key={ticket.id} className="bg-white rounded-3xl overflow-hidden relative">
              ...
              <div className="bg-gray-100 p-6 flex flex-col items-center justify-center gap-4">
                <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-200">
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${ticket.userId || 'vou-la-user'}:${ticket.id}`}
                    className="w-32 h-32"
                    alt="QR Code"
                  />
                </div>
                <p className="text-[10px] text-gray-400 font-mono tracking-[0.3em] font-black">{ticket.id.toUpperCase()}</p>
                {ticket.status === 'used' && (
                  <div className="absolute inset-0 bg-white/80 flex items-center justify-center backdrop-blur-[2px]">
                    <span className="text-red-500 font-black border-4 border-red-500 px-6 py-3 rounded-2xl rotate-[-15deg] text-2xl scale-110 shadow-xl">UTILIZADO</span>
                  </div>
                )}
              </div>
            </div>
