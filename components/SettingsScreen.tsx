...
import { X, Moon, Bell, Shield, Lock, LogOut, ChevronRight, UserMinus, Palette, Globe, HelpCircle, FileText, Smartphone, Database, RefreshCw, Edit3, KeyRound, AlertTriangle, ArrowLeft, Camera, Instagram, Twitter, Video, Sparkles } from 'lucide-react';
...
interface SettingsScreenProps {
    ...
    onUpdateSettings?: (settings: PrivacySettings, theme?: 'purple' | 'neon' | 'cyan' | 'pink' | 'light' | 'black') => void;
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ user, onClose, onLogout, onDeleteAccount, onUpdateSettings, onUpdateUser }) => {
    ...
    const [appMode, setAppMode] = useState<'light' | 'dark' | 'black'>(user.appMode || 'dark');
    const [accentColor, setAccentColor] = useState<'purple' | 'neon' | 'cyan' | 'pink'>(user.accentColor || 'neon');
    ...
    const handleModeChange = (newMode: 'light' | 'dark' | 'black') => {
        trigger('medium');
        setAppMode(newMode);
        document.documentElement.setAttribute('data-mode', newMode);
    };

    const handleAccentChange = (newColor: 'purple' | 'neon' | 'cyan' | 'pink') => {
        trigger('medium');
    ...
    const handleSaveSettings = () => {
        ...
        if (onUpdateSettings) {
            onUpdateSettings(settings); // Only privacy settings
        }
        if (onUpdateUser) {
            onUpdateUser({
                appMode: appMode,
                accentColor: accentColor,
                // The 'theme' property in User might be a legacy field,
                // if it's still used, it should reflect the accentColor or appMode.
                // For now, let's assume accentColor is the primary 'theme' if needed.
                theme: accentColor // Legacy fallback
            });
        }
        onClose();
    };
    ...
    // Detect Changes
    const hasSettingsChanges = JSON.stringify(settings) !== JSON.stringify(user.settings) || appMode !== user.appMode || accentColor !== user.accentColor;
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[var(--background)] flex flex-col animate-[slideUp_0.3s_ease-out]">
            ...
            <div className="flex-1 overflow-y-auto pb-44 scroll-container bg-[var(--bg-default)]">
                <div className="p-4 space-y-8">
                    ...
                    <section>
                        ...
                        <div className="bg-[var(--bg-card)] p-5 rounded-2xl border border-[var(--border-default)] overflow-hidden shadow-sm space-y-6">
                            <div>
                                <div className="flex items-center gap-3 mb-4">
                                    <Palette className="w-5 h-5 text-fuchsia-400" />
                                    <span className="text-white font-bold text-sm">Base do App</span>
                                </div>
                                <div className="flex gap-4">
                                    <ThemeOption
                                        color="#F8FAFC"
                                        label="BRANCO"
                                        selected={appMode === 'light'}
                                        onClick={() => handleModeChange('light')}
                                    />
                                    <ThemeOption
                                        color="#001217"
                                        label="DARK"
                                        selected={appMode === 'dark'}
                                        onClick={() => handleModeChange('dark')}
                                    />
                                    <ThemeOption
                                        color="#000000"
                                        label="PRETO"
                                        selected={appMode === 'black'}
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <span className="text-white font-bold text-sm">Cores de Destaque</span>
                                </div>
                                <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-none">
                                    <ThemeOption
                                        color="#ccff00"
                                        label="NEON"
                                        selected={accentColor === 'neon'}
                                        onClick={() => handleAccentChange('neon')}
                                    />
                                    <ThemeOption
                                        color="#c026d3"
                                        label="ROXO"
