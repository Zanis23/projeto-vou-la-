...
export const PlaceCard: React.FC<PlaceCardProps> = React.memo(({
            ...
            <div className="h-full bg-[#0B0F19] overflow-y-auto scroll-container overscroll-contain pb-safe">
                ...
                <div className="px-5 xs:px-6 -mt-4 relative z-10 space-y-6 pb-40">
                    ...
                    <button
                        ...
                        disabled={isCheckedIn || checkInState !== 'idle'}
                        className={`w-full py-5 xs:py-6 rounded-2xl xs:rounded-[2rem] font-black uppercase text-sm xs:text-base tracking-widest flex items-center justify-center gap-3 shadow-2xl transition-all active:scale-[0.97]
                        ${isCheckedIn ? 'bg-green-600 text-white' :
                                checkInState === 'success' ? 'bg-green-500 text-white animate-[pop_0.4s_ease-out]' :
                                    checkInState === 'checking' ? 'bg-indigo-600 text-white' :
                                        'bg-[var(--primary)] text-[var(--on-primary)] shadow-[0_0_25px_var(--primary-glow)]'}`}
                    >
                        {checkInState === 'checking' ? (
                            <><Loader2 className="w-5 h-5 animate-spin" /> VERIFICANDO GPS...</>
                        ) : checkInState === 'success' ? (
                            <><CheckCircle2 className="w-5 h-5 xs:w-6 xs:h-6" /> TUDO OK!</>
                        ) : isCheckedIn ? (
                            <><CheckCircle2 className="w-5 h-5 xs:w-6 xs:h-6" /> TÔ LÁ!</>
                        ) : (
    ...
    return (
        <div
            ...
            className={`group relative bg-[#131620] rounded-[2.5rem] p-3 border border-slate-800/60 shadow-[0_20px_50px_rgba(0,0,0,0.5)] active:scale-[0.98] transition-all cursor-pointer mb-4 overflow-hidden
        ...
        >
            ...
            <div className="flex gap-5 items-center relative z-10">
                {/* Image Container */}
                <div className="relative w-32 h-32 xs:w-36 xs:h-36 shrink-0 rounded-[2.2rem] overflow-hidden bg-slate-900 border border-white/5 shadow-2xl">
                    ...
                    <img
                        ...
                        className={`w-full h-full object-cover transition-all duration-[1.5s] group-hover:scale-110 ${imgLoaded ? 'opacity-100' : 'opacity-0'}`}
                        alt={place.name}
                    ...
                    {/* Premium Rank Badge */}
                    {rank && rank <= 3 && (
                        <div className={`absolute top-0 left-0 px-3 py-1.5 rounded-br-2xl text-[10px] font-black uppercase flex items-center gap-1.5 shadow-lg
                            ${rank === 1 ? 'bg-gradient-to-br from-amber-400 to-orange-600 text-black' :
                                rank === 2 ? 'bg-gradient-to-br from-slate-300 to-slate-500 text-black' :
                                    'bg-gradient-to-br from-orange-800 to-amber-900 text-white'}`}>
                            {rank === 1 ? <Crown className="w-3.5 h-3.5 fill-current" /> :
                                rank === 2 ? <Star className="w-3.5 h-3.5 fill-current" /> :
                    ...
                    {/* Occupancy Badge */}
                    <div className="absolute bottom-2 right-2 bg-black/60 backdrop-blur-xl rounded-2xl px-3 py-1.5 flex items-center gap-2 border border-white/10 shadow-xl">
                        <div className={`w-2 h-2 rounded-full shadow-[0_0_8px_currentColor] ${isHot ? 'bg-red-500 animate-pulse text-red-500' : 'bg-green-500 text-green-500'}`}></div>
                        <span className="text-[10px] font-black text-white italic tracking-tighter">{place.capacityPercentage}% <span className="text-[8px] opacity-60 not-italic uppercase ml-0.5">Lotado</span></span>
                    </div>
                </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center gap-2 py-2">
                    <div className="flex justify-between items-start">
                        <div className="min-w-0">
                            <span className="text-[8px] font-black text-[var(--primary)] uppercase tracking-[0.2em] mb-1 block opacity-80">{place.type}</span>
                            <h3 className="text-2xl xs:text-3xl font-black text-white italic truncate leading-[0.85] uppercase tracking-tighter mb-1 drop-shadow-md group-hover:text-[var(--primary)] transition-colors">{place.name}</h3>
                        </div>
                        <button
                            onClick={(e) => { e.stopPropagation(); onToggleSave?.(place.id); }}
                            className={`p-2 rounded-xl transition-all shadow-lg active:scale-90 border ${isSaved ? 'bg-[var(--primary)]/10 border-[var(--primary)]' : 'bg-slate-800/40 border-slate-700/50'}`}
                        >
                            <Bookmark className={`w-5 h-5 ${isSaved ? 'fill-[var(--primary)] text-[var(--primary)]' : 'text-slate-500'}`} />
                        </button>
                    </div>

                    <div className="flex items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-wider bg-slate-800/30 w-fit px-3 py-1.5 rounded-xl border border-white/5">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        <div className="flex -space-x-2">
                            {[1, 2, 3].map(i => (
                                <div key={i} className="w-6 h-6 rounded-full border-2 border-[#131620] bg-slate-800 flex items-center justify-center overflow-hidden">
                                    <img src={`https://i.pravatar.cc/100?u=${place.id}${i}`} className="w-full h-full object-cover" alt="" />
                                </div>
                            ))}
                            <div className="w-6 h-6 rounded-full border-2 border-[#131620] bg-slate-800 flex items-center justify-center text-[7px] font-black text-slate-400">+12</div>
                        </div>
                        <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest ml-1">Galera On</span>
                    </div>
                </div>

                <div className="flex flex-col items-center gap-4 py-2">
                    <div className="bg-slate-800/50 p-2 rounded-full border border-slate-700 group-hover:border-[var(--primary)] transition-colors">
                        <ChevronRight className="w-6 h-6 text-slate-600 group-hover:text-[var(--primary)] transition-all group-hover:translate-x-0.5" />
                    </div>
                    {isHot && (
            ...
            {/* Premium Glow Footprint */}
            <div className={`absolute -bottom-10 -right-10 w-32 h-32 blur-[40px] opacity-20 rounded-full pointer-events-none
                ${rank === 1 ? 'bg-amber-500' : rank === 2 ? 'bg-slate-400' : rank === 3 ? 'bg-orange-800' : 'bg-[var(--primary)]'}`}></div>
        </div>
    );
});
