...
export const Ranking: React.FC<RankingProps> = ({ currentUser }) => {
  // Generate a leaderboard that includes the current user dynamically
  ...
  return (
    <div className="p-4 pb-24 min-h-screen bg-[var(--background)] pt-safe transition-colors duration-500">
      ...
      {/* Top 3 Podium (Optional visual, kept simple list for now but styled) */}
          ...
          return (
            <div 
              ...
              className={`relative flex items-center p-4 rounded-2xl border transition-all
                ${isMe 
                    ? 'bg-[var(--surface)] border-[var(--primary)] shadow-[0_0_15px_var(--primary-glow)] scale-[1.02]' 
